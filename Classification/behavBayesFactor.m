...
for sub = 1:numFolds
    ...
    [B_both, lambda] = logReg(trainDataBoth, trainLabels, [1 10], false);
    fprintf('Lambda both = %d\n', lambda)
    
    likelihood_behav = computeLR_likelihood(B_behav, testDataBehav, testLabels);
    disp(likelihood_behav)
    likelihood_both = computeLR_likelihood(B_both, testDataBoth, testLabels);
    ...
    P = [testDataBoth ones(size(testLabels, 1), 1)]*B_both;
    P = exp(P)./(1 + exp(P));
    Yhat = double(P > 0.5);
